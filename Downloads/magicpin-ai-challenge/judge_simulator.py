...
BOT_URL = __import__("os").environ.get("BOT_URL", "http://localhost:8090")

# Choose your LLM provider: "openai", "anthropic", "gemini", "deepseek", "groq", "ollama", "openrouter"
LLM_PROVIDER = __import__("os").environ.get("LLM_PROVIDER", "openai")

# Your API key (paste your key here)
LLM_API_KEY = __import__("os").environ.get("LLM_API_KEY", "")  # <-- PUT YOUR API KEY HERE

# Model to use (leave empty for default, or specify like "gpt-4o", "claude-3-5-sonnet-20241022", etc.)
LLM_MODEL = __import__("os").environ.get("LLM_MODEL", "")  # <-- Optional: specify model or leave empty for default

# For Ollama only: local server URL
OLLAMA_URL = __import__("os").environ.get("OLLAMA_URL", "http://localhost:11434")

# Which test to run by default
TEST_SCENARIO = __import__("os").environ.get("TEST_SCENARIO", "all")
...
# Constants
TIMEOUT_LLM = int(os.environ.get("TIMEOUT_LLM", "45"))
...
class AnthropicProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        body_dict = {"model": self.model, "max_tokens": LLM_MAX_TOKENS,
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        req = urlrequest.Request(
            ...
            data=json.dumps({"model": self.model, "messages": messages,
                            "temperature": 0.2, "max_tokens": LLM_MAX_TOKENS}).encode("utf-8"),
            headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"}
        )
        resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
        data = json.loads(resp.read().decode("utf-8"))
...
class OllamaProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        resp = urlrequest.urlopen(req, timeout=max(90, TIMEOUT_LLM))
        data = json.loads(resp.read().decode("utf-8"))
